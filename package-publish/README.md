...
It is a `tool` package, so it runs in the person's own fence with the person's own agent-held ssh key: the registry's own authentication decides who may publish, and this package decides nothing about that. It is plain ECMAScript with no build step, and it has one dependency: `@thetis/runtime/lib/versions`, the single comparison it and `@thetis/marketplace` both decide "is this newer" with. Nothing is fetched into a fence to satisfy it -- a shipped package is linked into a userspace rather than installed with npm, which is the same arrangement `@thetis/tool-exec` already has with `@thetis/runtime/lib`.
...
| `sameRepository(a, b)`, `repoKey(url)`, `slugOfUrl(url)` | Two git urls compared as repositories, and the name a url suggests. Re-exported from `@thetis/runtime/lib/git-url`, the one normal form the installation has: the system fence's repository keys are matched to their repository by the same function, so a target and a key can never disagree about which repository a url names. |
...
| `compareVersions(a, b)`, `isNewer(a, b)` | Re-exported from `@thetis/runtime/lib/versions`, the same function `@thetis/marketplace` uses. Lenient and total: the version a publish is measured against comes out of somebody else's manifest in a registry and is ordered rather than refused. |
...
A fork without a build script publishes the runtime it already runs. If its `main` is under a build directory such as `dist/`, that directory is included even when the registry ignores build outputs; the dry run lists those files too. Other ignored files stay ignored. Before committing a package without a build step, the publisher checks that its entrypoint is present in the staged tree. A skill pack with no entrypoint needs no runtime files.
...
| `lib/semver.js` | What may be published and the next version along. The ordering is `@thetis/runtime/lib/versions`, re-exported. |
