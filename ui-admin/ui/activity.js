...
export function mountActivity(ext, root) {
  ...
  function draw() {
    ...
    put(
      wrap,
      el("div", { class: "toolbar" }, heading("Activity", `${rows.length} newest rows`), el("div", { class: "toolbar-gap" }), pick, reload),
      table(
        [
        ...
        rows,
        { rowKey: (r) => r.at + r.kind + (r.target || ""), empty: "Nothing recorded yet." }
      )
    );
  }
