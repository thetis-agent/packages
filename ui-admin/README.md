...
| `mounts-list` | `mountsList` | `user?` | `mounts.list`: one person's mounts, or everyone's. |
| `mounts-set` | `mountsSet` | `user`, `mounts` | `mounts.set`: replaces that person's list. At most 32 entries, each an absolute normalized path that is not `/`, mode `rw` or `ro`, no path twice. The kernel closes the person's fence. |
...
- **Mounts**: one table of every person's mounts with an **Unbind** button per row, and a **Bind directory** form: the person, the host path, the mode. Every change sends that person's whole list; the page says their fence reopens and their services restart.
...
| File | Content |
|---|---|
| `package.json` | The five `panel` entries and the eleven commands. |
