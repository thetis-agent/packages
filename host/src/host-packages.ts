...
import { HOST_TYPE, type HostEnv, type HostMethod, type Manifest } from "@thetis/contracts";
import type { KernelConfig } from "@thetis/kernel";
import { CodedError } from "@thetis/lib/error";
import { findPackage } from "./store.js";

/** The kernel's `HostExtensions`, spelled here because `@thetis/kernel` exports only `KernelServices`. `kernelOf` binds it as `hosts`, which checks it is the same shape. */
