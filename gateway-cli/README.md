...
| `status` | What is running, and whether each fence is on the code that is on disk now. `--json` prints the raw report: `{ daemon: { startedAt, uptimeSecs, supervised, restartPolicy, codeAt, stale }, restart, workspaces: [{ user, openedAt, codeAt, stale, services }] }`, which the installer reads. |
