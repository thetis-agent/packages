...
A `panel` entry declared with `under` (a built-in section id such as `packages`, or `<package>#<id>`) is not a nav item: its module registers `{ mount, children }`, the nav lists what `children()` answers (`[{ id, label, note?, mark? }]`, `mark` being `err` or `warn` for a dot) beneath that section, and a click on one mounts the entry with `{ child, refresh }` in the second argument, `refresh()` asking the nav to read the children again. `@thetis/ui-admin` hangs one settings page per configured package under Packages this way.
