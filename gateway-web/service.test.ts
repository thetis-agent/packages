/** Prove two inherited identities chat through the headless lifted wire in distinct sandboxes; KS-004, ADR 0019. */
...
async function person(shared: Awaited<ReturnType<typeof serviceFixture>>, who: string, other?: string): Promise<string> {
  ...
  const client = await webClient(gateway.socket);
  try {
    await client.send({ type: 'hello', person: 'forged' }); const hello = await client.next(); assert.deepEqual(hello['user'], { name: who });
