 ...
 *
 * What it says about a package no registry carries is the index's own statement and nothing more: the
 * index is built from the registries this installation mirrors, so its silence is a fact about what is
 * mirrored here, not about the world. Whether this person published it anyway, to a target nothing here
 * mirrors, is a different question with a different answer -- `publish_targets` asked about one package --
 * and it costs a fetch of every registry per package asked about, which a table of twenty rows cannot
 * spend. The package's own page in the marketplace asks it and says so, and the hint points there. */
...
export function mountPackages(root, { user }, shell) {
  ...
  const aheadBadge = (r) => {
    ...
    if (!a) return null;
    // Dim for a package the index does not carry, which is a quiet fact and true of most of them at once on
    // a maintainer's machine; warn for a version here that is past what a registry holds, which is a gap
    // between what this installation runs and what anybody else can get. Not "never published": the index
    // covers the registries this installation mirrors and nothing else, so its silence is a fact about what
    // is mirrored here. This is the sentence `thetis packages outdated` prints and the one the marketplace
    // card carries, word for word. See `aheadBadge` in ui-marketplace.
    return a.state === "unpublished" ? badge("no registry here lists it", "dim") : badge(`${a.version} here, ${a.published} published`, "warn");
  };
  ...
  function drawDetail() {
    ...
    if (ahead.get(row.name) && open) hints.push(ahead.get(row.name).state === "unpublished" ? `No registry here lists ${row.name}. Its page in the marketplace says whether it went to a registry this installation does not mirror, and is where Publish is.` : `${ahead.get(row.name).version} is here and ${ahead.get(row.name).published} is what ${ahead.get(row.name).registry} holds. Publish is on its page in the marketplace.`);
    put(
      detailEl,
      card(
        el("code", {}, row.name),
        ...
        kv([
          ...
          ahead.get(row.name)?.state === "unpublished" && ["published", el("span", { class: "text-faint" }, "no registry here lists it")],
