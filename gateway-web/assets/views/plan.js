 ...
 * Installed through the built-in `ext`: a transcript renderer that turns any todo_* call into one
 * quiet line and keeps the store's plan for that session, and the `todo` chip in each pane's chat bar.
 *
...
import { el, onClickOutside, setHidden } from "../lib/dom.js";
...
/** The chip's popover, under the chip: closes on Escape or an outside click, like confirm() and the Picker. */
function showPopover(button, plan, onClose) {
...
/** Registers the plan pieces with the shell through the built-in ext. */
export function installPlan(ext) {
  const args = new Map(); // todo_* call id -> its args, held from tool.call to tool.result

  // The transcript renderer: a todo_* call draws nothing; its result keeps the plan and draws one line.
  ext.transcript((event, ctx) => {
    if (event.type === "tool.call" && isTodoTool(event.call?.name)) {
      args.set(event.call.id, event.call.args || {});
      return true;
    }
    if (event.type === "tool.result" && isTodoTool(event.name)) {
      const plan = parsePlan(event.result);
      const callArgs = args.get(event.id) || {};
      args.delete(event.id);
      if (!plan) return true;
      store.setPlan(ctx.session, plan);
      return ctx.el("div", { class: "msg is-note is-quiet" }, ctx.el("span", { class: "note-dot" }), ctx.el("span", {}, planLine(event.name, callArgs, plan)));
    }
    return null;
  });
  ...
  ext.chip("todo", {
    draw(button, { session }) {
      const plan = store.planOf(session);
      setHidden(button, !plan);
      button.classList.add("mono");
      button.title = "This conversation's todo plan";
      if (!plan) return;
      button.textContent = `todo ${plan.done}/${plan.total}`;
      button.classList.toggle("is-done", plan.allSettled);
    },
    open({ session, button }) {
      if (open) return open(); // a second click closes; the popover's own close clears `open`
      const plan = store.planOf(session);
      if (!plan) return;
      open = showPopover(button, plan, () => { open = null; });
    },
  });
  store.watch("plan", () => ext.redraw("todo"));
}
