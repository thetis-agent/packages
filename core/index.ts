...
export interface Options {
  ...
  modelOptions?: Extract<RequestEvent, { type: 'begin' }>['options'];
...
export class Loop {
  ...
  async #run(input: Input, options: Options, signal: AbortSignal): Promise<Result<Message, 'io' | 'budget' | 'provider'>> {
    ...
    try {
      ...
      const maximum = options.maxIterations ?? defaults.iterations;
      for (let iteration = 1; iteration <= maximum; iteration++) {
        ...
        if (iteration === maximum) state.reason = 'limit';
      }
      ...
      const stopped: unknown = signal.reason; const boundary = signal.aborted && stopped instanceof Error && stopped.name === 'BoundaryFailure';
      if (report.exhausted || boundary) state.reason = 'crash';
      this.#emit(state, options, 'end', { reason: state.reason, iterations, compactions: state.compactions, ...(boundary ? { code: stopped.message } : {}) });
  ...
  async #iteration(state: TurnState, options: Options, signal: AbortSignal): Promise<Result<boolean, 'io' | 'budget' | 'provider'>> {
    ...
    if (refreshed) {
      retrieved.entries = retrieved.entries.filter(entry => !options.excludedSkills?.includes(entry.id));
  ...
  #request(prefix: Prefix, history: Message[], options: Options): RequestEvent[] {
    ...
    return [ { type: 'begin', id: `${options.conversation}-${head}`, model: options.model, ...(options.modelOptions ? { options: structuredClone(options.modelOptions) } : {}), cache: { prefixThrough: frames.filter(frame => frame.type === 'message').length - 1 } }, ...frames,
