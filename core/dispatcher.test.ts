...
await test('TE-015 deadline refuses the call while its later pending notice remains admissible', async () => {
  const f = await loopFixture(); const clock = new ManualClock();
  const completed = Promise.withResolvers<undefined>();
  const stage: Stage = { source: 'background', offer: () => Promise.resolve([definition]), async call(request) {
    await clock.wait(100);
    assert.ok(f.loop.notice('background', { handle: 'pending-1', content: [{ type: 'text', text: 'late completion' }] }).ok);
    completed.resolve(undefined); return { id: request.id, ok: true, pending: { handle: 'pending-1' } };
  } };
  try {
    const dispatcher = new Dispatcher([stage], schemas, clock); await dispatcher.offer({ mode });
    const called = dispatcher.call(call, new SpillSink(f.directory, 'deadline'));
    clock.advance(50); const result = await called; assert.equal(result.error?.code, 'deadline');
    clock.advance(50); await completed.promise;
    assert.equal(f.conversation.project().history.length, 0);
    assert.ok((await f.loop.turn({ text: 'next boundary', attachments: [] }, f.options, new AbortController().signal)).ok);
