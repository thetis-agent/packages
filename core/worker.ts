...
const startupTiming = { imports: performance.now(), schemas: 0, checks: 0, packages: 0, control: 0, cache: getCompileCacheDir() ?? null };
const schemas = new Schemas(); await schemas.load(); startupTiming.schemas = performance.now();
const check = await validator<Setup>(schemas, 'setup'); const command = await validator<WorkerCommand>(schemas, 'workerCommand');
startupTiming.checks = performance.now();
...
async function initialize(value: Setup): Promise<void> {
  ...
  startupTiming.control = performance.now();
  flushCompileCache(); send({ type: 'ready', sources: stages.map(stage => stage.source), startupTiming: { ...startupTiming, ...diagnostics() } });
}
