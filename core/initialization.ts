...
export class Initializer {
  ...
  async shutdown(deadlineMs: number): Promise<Result<void>> {
    this.#stopping = true; const worker = this.#worker;
    if (!worker) return { ok: true, value: undefined };
    const stopped = new Promise<boolean>(resolve => { worker.once('exit', () => { resolve(true); }); });
    const timer = new AbortController();
    try {
      worker.postMessage({ type: 'shutdown' });
      const finished = await Promise.race([stopped, this.#clock.wait(deadlineMs, timer.signal).then(() => false)]);
      if (!finished) { await worker.terminate(); return failure('deadline', 'The handler shutdown exceeded its bounded wait.'); }
      this.#worker = undefined; return { ok: true, value: undefined };
    } catch { return failure('io', 'The handler shutdown command could not be delivered.'); }
    finally { timer.abort(); }
  }
