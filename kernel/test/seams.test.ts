...
import { CONFIG_TIERS, defaultConfig } from "../src/config.js";

const PACKAGES = resolve(dirname(fileURLToPath(import.meta.url)), "../../..");
const source = (rel: string): string => readFileSync(resolve(PACKAGES, rel), "utf8");
...
test("the fence answers exactly these operations", () => {
  const text = source("userspace-agent/src/agent.ts");
...
test("a fence may ask the kernel exactly these methods", () => {
  assert.deepEqual(cases("kernel/src/rpc.ts"), [
    "auth.authenticate", "auth.login", "auth.logout",
    "config.effective", "config.set", "config.show", "config.unset",
    "models",
    "packages.delete", "packages.install", "packages.list", "packages.uninstall",
    "providers.call",
...
test("the operator table is exactly these methods, plus host packages under host.<name>.<export>", () => {
  assert.deepEqual(cases("kernel/src/control.ts"), [
    "config.get", "config.list", "config.reload", "config.set", "config.show", "config.unset",
    "fence.reload",
    "journal.tail",
    "models",
    "packages.install", "packages.installEveryone", "packages.list", "packages.promote", "packages.uninstall",
    "ping",
  ...
  assert.match(source("kernel/src/control.ts"), /"host\."/, "host packages are dispatched by prefix");
});

test("the door routes exactly the login page and each person's gateway", () => {
  const text = source("door/src/index.ts");
...
test("contracts export strings and nothing else at runtime", async () => {
  const mod = (await import("@thetis/contracts")) as Record<string, unknown>;
