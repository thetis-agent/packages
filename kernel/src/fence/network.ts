...
export function startEgress(pid: number, log: (line: string) => void): Promise<Egress> {
  return new Promise((done, fail) => {
    const child: ChildProcess = spawn(SLIRP, ["--configure", "--disable-host-loopback", "--enable-sandbox", "--enable-seccomp", "--ready-fd=3", String(pid), "tap0"], { stdio: ["ignore", "ignore", "pipe", "pipe"] });
    let settled = false;
    const finish = (err?: Error) => {
      ...
      if (err) {
        child.kill("SIGKILL");
        fail(err);
      } else done({ pid: child.pid!, stop: () => child.kill("SIGTERM") });
    };
