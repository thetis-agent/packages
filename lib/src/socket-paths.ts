...
export const HOME_SOCKETS: HomeSocket[] = [
  // @thetis/host: the operator channel every `thetis` command is a client of.
  { what: "the control socket", path: (home) => join(home, "thetis.sock") },
  // @thetis/sandbox: the per-fence ssh agent, opened by `ssh-agent` itself, which is the strictest opener here.
