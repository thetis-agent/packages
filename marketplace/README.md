...
| (`src/versions.ts`) | Gone to `@thetis/lib/versions`, so the publisher and the marketplace order versions with one function. `src/index.ts` re-exports it, and the cases in `test/marketplace.test.ts` still run against it. |
