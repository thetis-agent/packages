...
In mode `bwrap` the agent sees the operating system read-only, its own userspace root read-write, the shared directory (writable for the system userspace, read-only for everyone else), the package tree and the promoted packages read-only, its mounts, and its own cgroup read-only under `/sys/fs/cgroup`. It has its own user, PID, IPC, and UTS namespaces, no capabilities, cannot make a nested user namespace, and dies with the kernel. It does not see `$THETIS_HOME`, other userspaces, `/home`, the host `/tmp`, or any other fence's cgroup.

The cgroup bind is what lets an agent tell an OOM kill from a transient failure: it reads its real `memory.max`, `memory.current` and `memory.events` (`oom_kill`) instead of guessing from the host's free memory. The destination mirrors the fence's own `/proc/self/cgroup` line — `/sys/fs/cgroup/<that path>`, not `/sys/fs/cgroup` itself, because a runtime resolves its group by appending that line to the mount point, and the group bound at the mount root makes the concatenation name a directory that does not exist. .NET then aborts at random inside the fence (`munmap_chunk(): invalid pointer`). An agent reads its own limits the same way: `/proc/self/cgroup`, then that path under `/sys/fs/cgroup`. It is `--ro-bind-try`, so a kernel without a delegated cgroup, a cgroups v1 host, or mode `none` simply gets no bind — this never keeps a fence from starting. `/proc/meminfo` still reports the host's memory; that would need a FUSE layer such as `lxcfs` and is out of scope. See docs/03-fence.md section 3.3.
...
| `limits` | `memoryMb`, `pids`, `cpuPercent` | A cgroup v2 group per fence, bound read-only inside it at the path `/proc/self/cgroup` names, under `/sys/fs/cgroup`. Needs the kernel process in a delegated cgroup; otherwise the fences run unlimited, get no bind, and the kernel logs `[fence] resource limits off` once. |
...
| `src/cgroup.ts` | `Cgroups`, `fenceMount`. Per-fence limits under the kernel's delegated cgroup. `fenceDir` names the group on the host, `fence` adds where the fence has to see it: the mount point plus the group's path relative to the cgroup filesystem root, which is what `/proc/self/cgroup` reports inside. Only this file spells a cgroup path. |
