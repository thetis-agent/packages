...
await test('the surface block matches contract/surface and declares only paths this package serves', async () => {
  ...
  // A plan is drawn in the rail, never in the transcript: the tool calls that build it are the
  // agent's bookkeeping, and gateway-web's own tool card says everything a reader wants about those.
  assert.deepEqual(declared.renderers ?? [], [], 'this package claims no transcript row kind.');
  assert.deepEqual(Object.keys(isObject(raw['provides']) ? raw['provides'] : {}), ['panel/todo']);
  // A command with no panel to send it is refused by panels.ts, by name, and the package goes with it.
  assert.deepEqual((declared.commands ?? []).map(command => command.verb), ['plan', 'tick']);
});
...
await test('the import graph is closed, and only the entry module reaches for the seam', async () => {
  ...
  const entries = ['surface/panel.js'];
