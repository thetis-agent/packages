---
name: hosting
description: "Hosting Microsoft Orleans 10 on .NET 10: silos and clients, Host.CreateApplicationBuilder with UseOrleans(ISiloBuilder) and UseOrleansClient(IClientBuilder), UseLocalhostClustering for local development, the NuGet packages (Microsoft.Orleans.Server, Microsoft.Orleans.Client, Microsoft.Orleans.Sdk, provider packages), endpoints (ConfigureEndpoints, EndpointOptions, silo port 11111, gateway port 30000, AdvertisedIPAddress), ClusterOptions (ClusterId, ServiceId), the silo lifecycle stages (ServiceLifecycleStage), ILifecycleParticipant<ISiloLifecycle>, startup tasks and BackgroundService, co-hosting a silo inside ASP.NET Core, the co-hosted and external IClusterClient, IGrainFactory, client connection retry, graceful shutdown, logging, the options classes, and the Aspire integration. Use when you set up a new silo or client project, wire Orleans into a web host, pick ports and cluster ids, run something at silo startup, or need the right builder call or option class name."
metadata:
