---
name: best-practices
description: "Best practices for Microsoft Orleans 10 applications on .NET, from the official best-practices, FAQ, design-principles, and applicability pages: when Orleans fits (many independent loosely coupled entities) and when it does not, grain sizing and hot-spot avoidance, async everywhere and no blocking, short grain calls, small state, immutable messages and [Immutable], fan-out with Task.WhenAll, stateless workers and staged aggregation, avoiding chatty grain graphs and bottleneck grains, reentrancy trade-offs, deadlock avoidance and call-chain reentrancy, request timeouts, idempotency and retries, exceptions across grain calls, RequestContext, logging and telemetry, testing with TestCluster and InProcessTestCluster, at-most-once delivery and other runtime guarantees and non-guarantees, project structure (Abstractions, Grains, Silo, Client), and source generator and serializer constraints. Use when designing, reviewing, or optimizing Orleans grains and clusters."
metadata:
