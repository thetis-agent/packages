---
name: placement
description: "Microsoft Orleans 10 grain placement and activation movement: placement strategies and attributes (resource-optimized default since 9.2, ResourceOptimizedPlacementOptions, [RandomPlacement], [PreferLocalPlacement], [HashBasedPlacement], [ActivationCountBasedPlacement], [SiloRoleBasedPlacement]), changing the default, custom placement (PlacementStrategy, PlacementAttribute, IPlacementDirector), placement hints, placement filters (Orleans 9+) with silo metadata (UseSiloMetadata, RequiredMatchSiloMetadataPlacementFilter, PreferredMatchSiloMetadataPlacementFilter, IPlacementFilterDirector), grain migration (MigrateOnIdle, IGrainMigrationParticipant, [Immovable]), activation repartitioning (AddActivationRepartitioner) and rebalancing (AddActivationRebalancer), heterogeneous silos, and grain interface versioning ([Version], GrainVersioningOptions). Use when you decide where grains activate, balance load across silos, pin grains to zones or roles, move activations, or plan a rolling upgrade of grain interfaces."
metadata:
