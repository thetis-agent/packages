---
name: clustering
description: "Microsoft Orleans 10 clustering: the membership protocol and IMembershipTable, clustering providers with their NuGet packages and builder calls (UseLocalhostClustering, UseDevelopmentClustering, UseAdoNetClustering for SQL Server, PostgreSQL, MySQL, Oracle, UseAzureStorageClustering, UseCosmosClustering, DynamoDB, Consul, ZooKeeper, UseRedisClustering, Cassandra), ClusterId versus ServiceId, liveness tuning with ClusterMembershipOptions (ProbeTimeout, NumMissedProbesLimit, NumVotesForDeathDeclaration, IAmAlive), gateways, the grain directory (default distributed directory, strongly consistent directory, Redis, Azure Table, ADO.NET, the [GrainDirectory] attribute), and Kubernetes and Docker notes (Microsoft.Orleans.Hosting.Kubernetes, UseKubernetesHosting, pod labels, probes, advertised IP). Use when you choose or configure a membership provider, tune failure detection, debug silos that cannot find each other or clients that cannot connect, pick a grain directory, or deploy silos to containers or Kubernetes."
metadata:
