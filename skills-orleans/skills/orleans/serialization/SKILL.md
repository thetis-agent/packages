---
name: serialization
description: "Orleans serialization for Orleans 7 through 10: the Orleans.Serialization framework, [GenerateSerializer] and [Id(n)] on members, [Alias] on types and grain interface methods, member id scoping per inheritance level, version tolerance rules (add and remove members, widen numerics, never change ids or base classes), records and primary constructors, [Immutable] and Immutable<T> to skip deep copies, surrogates with [RegisterConverter] and IConverter/IPopulator for foreign types, delegating chosen types to System.Text.Json, Newtonsoft.Json or MessagePack (AddJsonSerializer, AddNewtonsoftJsonSerializer, AddMessagePackSerializer), exception serialization, the Microsoft.Orleans.Sdk source generator and ORLEANS00xx analyzer diagnostics, the grain storage serializer (IGrainStorageSerializer), migration from the Orleans 3.x serializer, and the error Could not find a codec for type. Use when defining message or state types, fixing codec errors, evolving deployed types, or choosing a storage serializer."
metadata:
