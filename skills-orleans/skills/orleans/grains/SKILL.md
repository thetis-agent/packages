---
name: grains
description: "Microsoft Orleans 10 grains: what a grain (virtual actor) is, grain interfaces and [Alias], grain identity and keys (Guid, long, string, compound, GrainId, [GrainType]), Grain base class versus IGrainBase (POCO grains), grain references and IGrainFactory, the grain lifecycle (OnActivateAsync, OnDeactivateAsync, activation collection, DeactivateOnIdle, DelayDeactivation, [KeepAlive]), the single-threaded turn-based execution guarantee, reentrancy ([Reentrant], [AlwaysInterleave], [ReadOnly], [MayInterleave], AllowCallChainReentrancy), [OneWay], [ResponseTimeout], RequestContext, CancellationToken in grain calls, grain call filters (IIncomingGrainCallFilter, IOutgoingGrainCallFilter), grain extensions (IGrainExtension), and [StatelessWorker] grains. Use when you write or review grain interfaces or grain classes on .NET 10, choose a key type, debug a grain deadlock or timeout, or need the exact Orleans attribute or API name for concurrency, cancellation, or lifecycle control."
metadata:
