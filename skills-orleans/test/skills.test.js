...
test("the package manifest declares a skill directory and nothing else in thetis", () => {
  ...
  assert.equal(m.name, "@thetis/skills-orleans");
...
test("the expected skills exist", () => {
  const expected = ["orleans", ...["grains", "hosting", "clustering", "placement", "persistence", "timers-reminders", "streams", "observers", "serialization", "transactions", "testing", "deployment", "migration", "best-practices"].map((n) => `orleans/${n}`)];
...
test("no skill in this pack is universal: its brief is in every prompt, its body is a fetch away", () => {
