...
export function create(config: Configuration, schemas: Schemas, clock: Clock, authority: Authority, budgets: Budgets, fetcher: Fetcher = fetch, scope: 'person' | 'deployment' = 'deployment'): ProviderEngine {
  return new ProviderEngine(new CompatibleVendor(config, schemas, clock, fetcher), authority, budgets, scope);
}
