...
export async function configure(input: Record<string, unknown>, key: string | undefined, authority: Authority, budgets: Budgets, schemas: Schemas, clock: Clock, scope: 'person' | 'deployment' = 'deployment'): Promise<Result<Provider>> {
  ...
  return { ok: true, value: create({ ...config.value, key }, schemas, clock, authority, budgets, undefined, scope) };
}
