...
export class ResponseState {
  ...
  #call(value: Record<string, unknown>): Result<ResponseEvent, 'provider'> {
    ...
    if (!previous && this.#calls.size >= settings.calls || Buffer.byteLength(id) > settings.identityBytes || Buffer.byteLength(name) > settings.identityBytes) return failure('provider', 'The vendor tool identities exceed their configured limit.');
